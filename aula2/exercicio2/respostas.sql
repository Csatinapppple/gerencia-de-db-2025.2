...
WHERE data BETWEEN '2023-06-10' AND '2023-06-11'
	AND co.status LIKE 'realizada';
...
WHERE DATE(con.data_hora) BETWEEN
	DATE('now', '+7 days') AND
	DATE('now', '+13 days') AND
	con.status NOT LIKE 'cancelada';
...
WHERE con.status LIKE 'cancelada'
	AND DATE(con.data_hora) >= DATE('now', 'start of month', '-1 month')
	AND DATE(con.data_hora) < DATE('now', 'start of month');

--16. Faturamento total por mês (mês/ano, total).

SELECT
    strftime('%Y-%m', p.data_pagto) AS mes_ano,
    SUM(p.valor) AS faturamento_total
FROM pagamentos p
WHERE p.status = 'pago'
  AND strftime('%Y', p.data_pagto) = '2023'
GROUP BY mes_ano;
...
SELECT 
    ...
    telefone
FROM clientes
WHERE strftime('%m', dt_nascimento) = strftime('%m', 'now');
...
ON an.id = con.animal_id
INNER JOIN consulta_procedimentos con_pro
ON con_pro.consulta_id = con.id
INNER JOIN procedimentos pro
ON con_pro.procedimento_id = pro.id
INNER JOIN clientes cli
ON cli.id = an.cliente_id
GROUP BY an.nome
HAVING pro.tipo NOT LIKE 'cirurgico';

--19. Última consulta realizada por cada animal (nome, data, veterinário).

SELECT
	an.nome as animal,
	MAX(DATE(con.data_hora)) as data,
	vet.nome as veterinario
FROM consultas con
INNER JOIN animais an
ON con.animal_id = an.id
INNER JOIN veterinarios vet
ON con.veterinario_id = vet.id
GROUP BY an.nome;
...
SELECT
	vet.nome as veterinario,
	COUNT(con.status) as quantidade
FROM consultas con
INNER JOIN veterinarios vet
ON con.veterinario_id = vet.id
GROUP BY vet.nome
HAVING con.status LIKE 'realizada'
	AND DATE(con.data_hora) BETWEEN DATE('now', 'start of month', '-1 month')
